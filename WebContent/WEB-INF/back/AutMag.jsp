...
<link rel="stylesheet" href="${path}/css/zTreeStyle/myZTree.css">
 ...
 */
 function allot(rid) {
		...
		var setting = {
		    ...
		    async: {
		        enable: true,
			    url:"${path}/Auth/allot.action?yyy="+rid,
				autoParam : ["id","name"],
 ...
 */
 function unAllot(rid) {
	 ...
	 var setting = {
			    ...
			    async: {
			        enable: true,
				    url:"${path}/Auth/unallot.action?yyy="+rid,
					autoParam : ["id","name"],
//	 				contentType : "application/json;charset=utf-8",
//	 				otherParam: {"yyy":rid},
					dataFilter : function filter(treeId, parentNode, childNodes) {
...
var r_id;
	...
	function removeMenu(){
		$.ajax({
			type: "POST",
			url: "${path}/Auth/removemenu.action",
			contentType:"application/json;charset=utf-8",
